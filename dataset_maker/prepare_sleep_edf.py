...
drop_channels = ['EMG submental', 'Event marker', 'Temp rectal', 'Resp oro-nasal', 'EEG Fpz-Cz', 'EEG Pz-Oz', ]
# For both SC (Sleep Cassette) and ST (Sleep Telemetry) files
chOrder_standard = ['EOG horizontal']
...
def process_dataset(input_dir, output_dir):
    ...
    for subdir in ['sleep-cassette', 'sleep-telemetry']:
        search_dir = os.path.join(input_dir, subdir) if os.path.exists(os.path.join(input_dir, subdir)) else input_dir
        for dirName, _, fileList in os.walk(search_dir):
            for fname in fileList:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default=r"E:\dataset\SleepEDF\sleep-telemetry",
