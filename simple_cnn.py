...
class EEGDataset(Dataset):
    def __init__(self, data_dir, n_classes=None):
...
def main():
    ...
    parser.add_argument('--dataset', type=str, 
                        choices=['hmc', 'workload', 'sleepedf', 'eog'],
    ...
    train_dataset = EEGDataset(train_dir, config['n_classes'])
    eval_dataset = EEGDataset(eval_dir, config['n_classes'])
    test_dataset = EEGDataset(test_dir, config['n_classes'])
    ...
    print(f"Input shape: channels={n_channels}, sequence_length={seq_len}")
    ...
    try:
        ...
        # Add dataset-specific name to saved files
        plt.savefig(f'{args.dataset}_training_curves.png')
        torch.save(model.state_dict(), f'{args.dataset}_model.pt')
        print(f"Model saved to {args.dataset}_model.pt")
